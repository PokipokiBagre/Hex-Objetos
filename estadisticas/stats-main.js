...
window.addHexLogEntry = (nombre, amount, isExtra = false) => {
    ...
    if (isExtra) {
        log.extra.amount += amount; log.extra.finalHex = p.hex;
        log.order = log.order.filter(k => k !== 'extra'); log.order.push('extra');
    } else if (amount > 0) {
        log.pos.amount += amount; log.pos.finalHex = p.hex;
        log.order = log.order.filter(k => k !== 'pos'); log.order.push('pos');
    } else if (amount < 0) {
        log.neg.amount += Math.abs(amount); log.neg.finalHex = p.hex;
        log.order = log.order.filter(k => k !== 'neg'); log.order.push('neg');
    } else if (amount === 0) {
        if (log.order.length === 0) {
            log.pos.amount = 0; log.pos.finalHex = p.hex; log.order.push('pos');
        }
    }
};

window.limpiarHexLog = () => { estadoUI.hexLog = {}; updateHexLogText(); };
window.copiarHexLog = () => { const textarea = document.getElementById('hex-log-textarea'); if (textarea) { textarea.select(); document.execCommand('copy'); alert("Log copiado al portapapeles exitosamente."); } };
...
function recalcularVidas(p, accion) {
    ...
    const postFisBase = p.afinidades?.fisica || 0; const postFis = calcFisTotal();
    const postMagBase = (p.afinidades?.energetica||0) + (p.afinidades?.espiritual||0) + (p.afinidades?.mando||0) + (p.afinidades?.psiquica||0); const postMag = calcMagTotal();

    const deltaCorazonesBaseFis = Math.floor(postFisBase / 2) - Math.floor(preFisBase / 2);
    if (deltaCorazonesBaseFis !== 0) p.vidaRojaMax = Math.max(0, (p.vidaRojaMax || 10) + deltaCorazonesBaseFis);

    const deltaCorazonesBaseMag = Math.floor(postMagBase / 4) - Math.floor(preMagBase / 4);
    if (deltaCorazonesBaseMag !== 0) p.baseVidaAzul = Math.max(0, (p.baseVidaAzul || 0) + deltaCorazonesBaseMag);

    const deltaCorazonesFisica = Math.floor(postFis / 2) - Math.floor(preFis / 2);
    if (deltaCorazonesFisica !== 0) p.vidaRojaActual = Math.max(0, (p.vidaRojaActual || 0) + deltaCorazonesFisica);

    const deltaCorazonesMagia = Math.floor(postMag / 4) - Math.floor(preMag / 4);
    if (deltaCorazonesMagia !== 0) p.vidaAzul = Math.max(0, (p.vidaAzul || 0) + deltaCorazonesMagia);
...
window.recalcularBases = () => {
    ...
    if(confirm(`¿Seguro que deseas RECALCULAR las vidas bases de ${estadoUI.personajeSeleccionado}?\n\nEsto pondrá el Límite Rojo a 10 (para aplicar la física pura), la Vida Azul a su equivalente mágico y lo curará al máximo.`)) {
        p.vidaRojaMax = 10; p.vidaRojaActual = calcularVidaRojaMax(p); p.vidaAzul = getMysticBonus(p); p.baseVidaAzul = p.vidaAzul; guardar(); repintarConScroll('detalle');
    }
};
...
window.modificarBuff = (statId, cantidad) => { const p = statsGlobal[estadoUI.personajeSeleccionado]; if(!p) return; recalcularVidas(p, () => { p.buffs[statId] = (p.buffs[statId] || 0) + cantidad; }); guardar(); repintarConScroll('detalle'); };
window.modBaseTop = (statId, cantidad) => { const p = statsGlobal[estadoUI.personajeSeleccionado]; if(!p) return; recalcularVidas(p, () => { p[statId] = Math.max(0, (p[statId] || 0) + cantidad); }); guardar(); repintarConScroll('op'); };
window.modBaseAfin = (statId, cantidad) => { const p = statsGlobal[estadoUI.personajeSeleccionado]; if(!p) return; recalcularVidas(p, () => { p.afinidades[statId] = Math.max(0, (p.afinidades[statId] || 0) + cantidad); }); guardar(); repintarConScroll('op'); };
window.modSpellTop = (statId, cantidad) => { const p = statsGlobal[estadoUI.personajeSeleccionado]; if(!p) return; recalcularVidas(p, () => { p.hechizos[statId] = (p.hechizos[statId] || 0) + cantidad; }); guardar(); repintarConScroll('op'); };
window.modSpellAfin = (statId, cantidad) => { const p = statsGlobal[estadoUI.personajeSeleccionado]; if(!p) return; recalcularVidas(p, () => { p.hechizos[statId] = (p.hechizos[statId] || 0) + cantidad; }); guardar(); repintarConScroll('op'); };
window.modSpellEffTop = (statId, cantidad) => { const p = statsGlobal[estadoUI.personajeSeleccionado]; if(!p) return; recalcularVidas(p, () => { p.hechizosEfecto[statId] = (p.hechizosEfecto[statId] || 0) + Math.max(-Math.abs(p.hechizosEfecto[statId] || 0), Math.min(Math.abs(p.hechizosEfecto[statId] || 0), cantidad)); }); guardar(); repintarConScroll('op'); };
window.modSpellEffAfin = (statId, cantidad) => { const p = statsGlobal[estadoUI.personajeSeleccionado]; if(!p) return; recalcularVidas(p, () => { p.hechizosEfecto[statId] = (p.hechizosEfecto[statId] || 0) + cantidad; }); guardar(); repintarConScroll('op'); };
...
window.ejecutarClonacion = (tipo) => {
    ...
    if (tipo === 'efectosExtras' || tipo === 'completo') { target.buffs = JSON.parse(JSON.stringify(source.buffs)); target.hechizosEfecto = JSON.parse(JSON.stringify(source.hechizosEfecto || {})); if (tipo !== 'completo') { const newMaxRojo = calcularVidaRojaMax(target); if (target.vidaRojaActual > newMaxRojo) target.vidaRojaActual = newMaxRojo; } }
    if (tipo === 'hex' || tipo === 'completo' || tipo === 'stats_puros') target.hex = source.hex;
    if (tipo === 'completo' || tipo === 'stats_puros') {
        target.vidaRojaActual = source.vidaRojaActual; target.vidaRojaMax = source.vidaRojaMax; target.vidaAzul = source.vidaAzul; target.baseVidaAzul = source.baseVidaAzul; 
        target.guardaDorada = source.guardaDorada; target.baseGuardaDorada = source.baseGuardaDorada; target.danoRojo = source.danoRojo; target.danoAzul = source.danoAzul; target.elimDorada = source.elimDorada;
        target.afinidades = JSON.parse(JSON.stringify(source.afinidades)); target.hechizos = JSON.parse(JSON.stringify(source.hechizos || {})); target.vex = source.vex;
...
window.ejecutarCreacionNPC = () => {
    ...
    const inputVrm = parseInt(document.getElementById('npc-vrm').value) || 0; const inputVa = parseInt(document.getElementById('npc-va').value) || 0;
    const inputVra = parseInt(document.getElementById('npc-vra').value) || 0; const guardaD = parseInt(document.getElementById('npc-gd').value) || 0;
    ...
    statsGlobal[nombre] = {
        ...
        vidaRojaActual: inputVra, vidaRojaMax: inputVrm, vidaAzul: inputVa, baseVidaAzul: inputVa, guardaDorada: guardaD, baseGuardaDorada: guardaD,
