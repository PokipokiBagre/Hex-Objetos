...
async function iniciar() {
    ...
    window.copyToClipboard = (id) => { 
        const area = document.getElementById(id); area.select(); document.execCommand('copy'); 
    };

    // ACTUALIZACIÓN EN TIEMPO REAL PARA CREACIÓN
    ...
    window.ejecutarSyncLog = () => {
        ...
        if (i === atob(_session)) { estadoUI.esAdmin = true; dibujarMenuOP(); window.mostrarPagina('op-menu'); } else alert("Error de redundancia.");
    };
    ...
    // GENERADOR DE LOG PARA EL EDITOR (DINÁMICO AL PULSAR + o -)
    window.hexMod = (j, o, c) => { 
        const tag = c > 0 ? "OO" : "OP"; 
